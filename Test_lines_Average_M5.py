...
# reading in an image
imageorig = mpimg.imread('test_images/frames_021.jpg')
...
def getSlopesAverage(houghlines, index):
    ...
    sum = 0
    for i in range(n):
        sum += math.atan(slopes[index[i]])
    return math.tan(sum/n)
...
def lines_average(img, rho, theta, threshold, min_line_len, max_line_gap):
    ...
    if slopeLeftLaneAve == 0:
        ...
        x_end_left = int((sizeImage[0]*alfa - bLeft)/slopeLeftLaneAve)
    y_end_left = int(sizeImage[0]*alfa)
    ...
    if slopeRightLaneAve == 0:
        ...
        x_end_right = int((sizeImage[0]*beta - bRight)/slopeRightLaneAve)
    y_end_right = int(sizeImage[0]*beta)
...
# Step 1: Gray scaling the image
grayscalimg = grayscale(image)
plt.imshow(grayscalimg, cmap='gray')
plt.show()

# Step 2: Gaussian smoothing
grayblure = gaussian_blur(grayscalimg, 5)
plt.imshow(grayblure,cmap='gray')
plt.show()

# Step 3: Canny detection
low_threshold = 80
high_threshold = 180
edgescanny = canny(grayblure, low_threshold, high_threshold)
...
region_selection = region_of_interest(edgescanny, vertices)
plt.imshow(region_selection, cmap='gray')
plt.show()

# Step 5: Get and Draw hough lines
rho = 2
theta = np.pi/180
threshold = 10
min_line_len = 30
max_line_gap = 15
Lines_edges = hough_lines(region_selection, rho, theta, threshold, min_line_len, max_line_gap)
mix = weighted_img(Lines_edges,imageorig, α=0.8, β=1.0, γ=0)
plt.imshow(mix, cmap='gray')
plt.show()

# Step 6: Average and extrapolate hough lines
rho = 2
theta = np.pi/180
threshold = 10
min_line_len = 20
max_line_gap = 20
Lines_average = lines_average(region_selection, rho, theta, threshold, min_line_len, max_line_gap)
plt.imshow(Lines_average)
plt.show()

# Step 7: mix orginal image awith average lines
image_final = weighted_img(Lines_average, imageorig, α=0.8, β=1.0, γ=0)
...
#     max_line_gap = 7
#     Lines_average = lines_average(region_selection, rho, theta, threshold, min_line_len, max_line_gap)
#     image_final = weighted_img(Lines_average, imageinitial, α=0.8, β=1.0, γ=0)
#     return image_final
#
#
